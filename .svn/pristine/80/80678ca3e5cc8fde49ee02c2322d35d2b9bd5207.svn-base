...
public abstract class MaspSimulationBehaviour extends SimpleBehaviour {
	...
	public final void action() {
		...
		if (!finished) {
			...
			if (blockTime <= 0) {
			  ...
			  a.stopSimulationTimeTardinessInforming(a);
				...
				informTardiness(tardiness);
				a.startSimulationTimeTardinessInforming(a);
				wakeupSimulationTime = now + simulatedTimePeriod;

				// blockTime = simulatedTimePeriod;

				/*
				 * System.out.println(a.getLocalName() + " com comportamento " +
				 * this + ">> periodo " + simulatedTimePeriod +
				 * " e rate LC >>> " + SimulationClockControl.getInstance()
			...
			// block((long) (simulatedTimePeriod /
	...
	public void informTardiness(double tardiness) {
		StatisticCollector.getInstance().insertData(tardiness,
				isPartSimulationModel);
